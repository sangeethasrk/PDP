...
(define (process-expr e d)
  (cond
    ...
    [(bool? e) (handle-bool (bool-op e) (bool-args e) d)]                   
...
(define (handle-algebra op args defs)
  (local
    ((define (process-args arg)
       (map (λ(l) (process-expr l defs)) arg)))
    (if (andmap number? (process-args args))
        (perform-algebraic-opn op (process-args args))
        (find-error-str (process-args args)))))

(define (find-error-str lst)
  (first
   (filter string? lst)))
...
(define (handle-bool op args defs)
  (local
    ((define (map-fn arg)
       (map (λ(l) (process-expr l defs)) args)))
