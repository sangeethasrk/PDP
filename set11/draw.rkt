...
(define (handleButtonDown x y w)
  (if (and (< 0 x TL-SIZE) (< 0 y (* 3 TL-SIZE)))
      (updateTool y w)
...
; checkForR/C: Coordinate World -> World
; Returns new world by updating toolbar based on selection
; Stratey: data decmposition on w : World
(begin-for-test
  (check-equal? (checkForR/C 45 INITIAL-WORLD)
                (make-world '() 'circ) 
                "Circle selection"))

(define (checkForR/C y w)
...
(define (handleButtonUp x y w) 
  (if (and (< 0 x TL-SIZE) (< 0 y (* 3 TL-SIZE))) w
...
(begin-for-test
  (check-equal? (render (make-world '() 'point))
                (overlay/align 
                 "left" "top" 
                 (overlay/offset POINTER-SEL 0 30 
                                 (overlay/offset RECTANGLE 0 20 CIRCLE)) MT)
  ...
  (check-equal? (render (make-world (list RECT-TEST) 'rect))
                (send RECT-TEST draw 
                      (overlay/align 
                       ...
                       (overlay/offset 
                        POINTER 0 30 
                        (overlay/offset RECTANGLE-SEL 0 20 CIRCLE)) MT))
                "Rectangle render")
  (check-equal? (render (make-world (list CIRC-TEST) 'circ))
                (send CIRC-TEST draw 
                      (overlay/align 
                       ...
                       (overlay/offset 
                        POINTER 0 30 
                        (overlay/offset RECTANGLE 0 20 CIRCLE-SEL)) MT))
                "Rectangle render"))
...
; Stratey: data decmposition on sel : SelTool
(begin-for-test
  (check-equal? (render-toolbar 'point)
                (overlay/offset 
                   POINTER-SEL 0 30 
                   (overlay/offset RECTANGLE 0 20 CIRCLE))
                "rendering toolbar"))
...
(define (render-toolbar sel)
  (cond
    ...
    [(rect? sel) (overlay/offset 
                  POINTER 0 30 
                  (overlay/offset RECTANGLE-SEL 0 20 CIRCLE))]
    [(circ? sel) (overlay/offset 
                  POINTER 0 30 
                  (overlay/offset RECTANGLE 0 20 CIRCLE-SEL))]))
