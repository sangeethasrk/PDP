...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname pretty) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp")))))
(require "extras.rkt")
(require rackunit)
(require racket/list)
...
(define (multi-line-print-prefix lst width op)
  (local
    (; multi-line-print-prefix/a : ListOf<String> Natural Lons -> ListOf<String>
     ; Renders the multi line infix representation of expressions as strings
     ; WHERE : expr-so-far is the processed part of the expression 
...
(define (render-prefix neloe width op)
  (cond
    [(empty? (rest neloe))
     (add-at-last CLOSE (expr->strings (first neloe) width))]
    [else
